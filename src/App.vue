<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <div v-else-if="trainingStore.isReady" class="min-h-screen flex flex-col">
      <header
        class="w-full bg-red-500 text-white shadow-md flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between px-6 py-4"
      >
        <div class="flex items-center gap-4">
          <img src="@/assets/logo.png" alt="Mascotte Gym JP" class="w-16 h-auto" />
          <div>
            <p class="text-2xl font-extrabold tracking-wide">Programme d'entraînement</p>
            <p class="text-sm text-red-100">Connecté : {{ authStore.user?.email }}</p>
          </div>
        </div>

        <Button variant="secondary" class="cursor-pointer" @click="handleLogout">
          Se déconnecter
        </Button>
      </header>
      ...
      <div class="flex flex-wrap justify-center gap-2 mt-6 px-4">
        <div
          ...
          :key="date.key"
          @click="trainingStore.setSelectedDate(date.fullDate)"
          class="cursor-pointer px-4 py-2 rounded-full border transition-all duration-200 text-sm sm:text-base"
          :class="trainingStore.selectedDate === date.fullDate
      ...
      <div class="w-full max-w-4xl mx-auto px-4 mt-6 space-y-4">
        <ExerciseCard
          ...
          :key="log.id"
          :exercise="trainingStore.allExercises.find(e => e.id === log.exercise_id)"
          :reps="log.reps"
          :weight="log.weight"
          :sets="log.sets"
      ...
      <footer class="w-full bg-gray-100 border-t py-4 text-center text-sm text-gray-600 mt-auto">
        © 2025 – Créé par
        <a
          href="https://github.com/Verdungo"
          target="_blank"
          rel="noopener"
